...
plot <- core_microbiome(physeq)
plot <- plot_theme_default(plot)

plot <- theme_cowplot(font_size = text_size) + theme(
...
if(what_detection=="relative"){
  plot <- plot + xlab("Relative Abundance (%)")
}
```
...
width_image_heatmap <- ceiling(nlevels(datacore$data$Taxa) / 5) + 2
height_image_heatmap <- 16
filename <- paste("Core_Heatmap_",label,"_",which_level,"_",what_detection,".pdf",sep="")

save_plot(plot, filename, width = width_image_heatmap, height = height_image_heatmap)
```
